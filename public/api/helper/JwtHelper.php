...
class JwtHelper
{
   ...
   */
  public function __construct(string $segredo = null, array $dados = [])
  {
   ...
   * @return bool
   */
  public function validarSegredo(string $segredo): bool
  {
    $part = explode(".", $this->token);
    $header = $part[0];
    $payload = $part[1];
   ...
   */
  public static function criarToken(string $segredo = null, array $dados = [])
  {
   ...
   * @return bool
   */
  public static function validarToken(string $token, string $segredo): bool
  {
    $part = explode(".", $token);
    $header = $part[0];
    $payload = $part[1];
   ...
   * @return object
   */
  public static function obterDados(string $token)
  {
    $part = explode(".", $token);
    $payload = $part[1];
    $payload = base64_decode($payload);
